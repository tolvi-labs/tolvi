...
const tolviLabsRoots = `{
  "roots": [
    {"role": "org", "workspace": "tolvi-labs", "path": "/vaults/marketing-site/vault"},
    {"role": "org", "workspace": "acme", "path": "/vaults/acme-shared/vault"},
    {"role": "org", "workspace": "isolated-org", "path": "/vaults/isolated-org/vault"},
    {"role": "product", "product": "birdie-os", "path": "/vaults/birdie-os-vault/vault"}
  ]
}`
...
func TestChainOrdersRootsNearestFirst(t *testing.T) {
	c := loadTolviLabs(t).Chain(Identity{Workspace: "birdie-os", Repo: "birdie-os-web", Product: "birdie-os"}, "/repo/vault")

	// birdie-os-web declares no matching org root, so the chain is repo + product.
...
func TestChainSelectsOrgRootByWorkspace(t *testing.T) {
	...
	if org.Path != "/vaults/marketing-site/vault" {
...
func TestTargetResolutionTable(t *testing.T) {
	...
	cases := []struct {
		...
		{"private decision routes to the org root", "decision", "private", "/vaults/marketing-site/vault"},
		{"public pattern stays in the repo vault", "pattern", "", "/repo/vault"},
		{"private pattern routes to the org root", "pattern", "private", "/vaults/marketing-site/vault"},
		{"a session always routes private", "session", "", "/vaults/marketing-site/vault"},
		{"an explicitly private session routes private too", "session", "private", "/vaults/marketing-site/vault"},
	}
...
func TestTargetPrefersProductOverOrgForPrivateDocs(t *testing.T) {
	...
	if got.Path != "/vaults/birdie-os-vault/vault" {
...
func TestAcceptsFiltersSharedRootsByRepo(t *testing.T) {
	...
	if c.Accepts(org, DocScope{Repo: "marketing-site"}) {
