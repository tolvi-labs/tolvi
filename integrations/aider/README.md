...
Aider is a **Tier 3 (conventions file)** integration in the [Tolvi integrations tier list](../README.md). The file ships a compact format-spec recap that should let Aider participate in vault read/write flows; it does not include the deeper behavioral guidance or worked examples of the Tier 1 [Claude Code skill](../../skills/tolvi/). If you find yourself wanting more, copy from there.
...
From the root of your project (the repo where you want Aider to know about Tolvi), copy the file in:

```bash
cp /path/to/tolvi-labs/tolvi/integrations/aider/CONVENTIONS.md CONVENTIONS.md
```

It is a copy rather than a symlink because you commit it, and a committed symlink into your own tolvi checkout would be broken for everyone else. To update, copy it again after pulling the tolvi repo.
...
```bash
rm CONVENTIONS.md
# and remove the `read: CONVENTIONS.md` entry from .aider.conf.yml
```
