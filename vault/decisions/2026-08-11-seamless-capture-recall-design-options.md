...
Brainstormed from a broader question (asked in a acme-site session): how to make using Tolvi feel seamless rather than something a user has to be trained on. That question decomposes into three separable pieces, only the first of which was explored here:
...
3. **acme-site positioning of a "zero-training" Tolvi** — depends on (1) and (2) existing first; not started.
