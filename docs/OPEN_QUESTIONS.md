...
10. **SDK type generation depends on the monorepo layout.** The `@tolvi-labs/sdk` package generates its TypeScript types from `spec/openapi.json` via `openapi-typescript ../spec/openapi.json -o src/types.gen.ts`. The relative `../spec/openapi.json` path assumes the SDK lives inside the tolvi monorepo. If the SDK is ever extracted to its own repository, `gen:types` must source the spec another way — likely either downloading a tagged release artifact (`curl -sSL https://github.com/tolvi-labs/tolvi/releases/download/server-vX.Y.Z/openapi.json`) or fetching from a running server via `/openapi.json`. No action needed pre-extraction; flagged so the constraint is captured rather than rediscovered later. Until extraction is on the table, the SDK README MUST note that `gen:types` is monorepo-local.

11. **`spec/openapi.json` is not CI-verified against server routes.** The committed OpenAPI document is generated from the live Fastify route schemas via `npm run openapi:dump`, but nothing in `.github/workflows/server.yml` runs that command and asserts no diff against the committed `spec/openapi.json`. A contributor who modifies a route's zod schema and forgets to re-dump silently desyncs the spec from reality, and downstream consumers (the Phase 5.A TypeScript SDK type generator, third-party client codegen) propagate the staleness. Likely fix: add a step to the server CI that runs `npm run openapi:dump` and `git diff --exit-code spec/openapi.json`, mirroring the drift check in the SDK workflow. Small change; gated only by deciding when to land it (probably alongside the SDK CI workflow itself).
